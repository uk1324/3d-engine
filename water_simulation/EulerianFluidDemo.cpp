...
void EulerianFluidDemo::setSmoke(i64 x, i64 y, Vec3 color) {
	smokeR(x, y) = color.x;
	smokeG(x, y) = color.y;
	smokeB(x, y) = color.z;
}
			...
			for (i64 xi = 2; xi < fluid.gridSize.x; xi += 5) {
				for (i64 yi = 2; yi < fluid.gridSize.y; yi += 5) {
					...
					for (i32 i = 0; i < stepCount; i++) {
						...
						if (i == stepCount - 1) {
							const auto direction = (pos - oldPos).angle();
							Dbg::drawLine(pos, pos - Vec2::oriented(direction + 0.4f) * 0.02f, Color3::WHITE, 0.005f);
							Dbg::drawLine(pos, pos - Vec2::oriented(direction - 0.4f) * 0.02f, Color3::WHITE, 0.005f);
						}
					}
				}
