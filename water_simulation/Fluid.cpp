...
float EulerianFluid::sampleVelX(Vec2 pos) {
	return sampleField(spanFrom(velX).asConst(), pos, Vec2(0.0f, cellSpacing / 2.0f));
}

float EulerianFluid::sampleVelY(Vec2 pos) {
	return sampleField(spanFrom(velY).asConst(), pos, Vec2(cellSpacing / 2.0f, 0.0f));
}
	...
	for (i64 x = 1; x < gridSize.x; x++) {
		for (i64 y = 1; y < gridSize.y; y++) {
			...
			if (!isWall(x - 1, y) && y < gridSize.y - 1) {
				...
				const Vec2 vel{ at(velX, x, y), avgVelY };
				const auto approximatePreviousPos = pos - vel * dt;
				at(newVelX, x, y) = sampleVelX(approximatePreviousPos);
			}
			...
			if (!isWall(x, y - 1) && x < gridSize.x - 1) {
				...
				const Vec2 vel{ avgVelX, at(velY, x, y) };
				const auto approximatePreviousPos = pos - vel * dt;
				at(newVelY, x, y) = sampleVelY(approximatePreviousPos);
			}
		}
	}
